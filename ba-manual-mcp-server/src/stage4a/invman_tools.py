...
async def get_all_invoices(paid: Annotated[Optional[bool], Field(description="Filter invoices by paid status")] = None,
    ...
    try:
        ...
        if jq_filter is not None and jq_filter != "":
            return jq.compile(jq_filter).input_([invoice.model_dump(mode="json") for invoice in invoices]).all()
        else:
            return invoices
...
async def create_invoice(invoice: Invoice,
    ...
    try:
        ...
        if jq_filter is not None and jq_filter != "":
            return jq.compile(jq_filter).input_value(created_invoice.model_dump(mode="json")).all()
        else:
...
async def get_all_sales_taxes(jq_filter: Annotated[Optional[str], Field(
    ...
    try:
        ...
        if jq_filter is not None and jq_filter != "":
            return jq.compile(jq_filter).input_([tax.model_dump(mode="json") for tax in sales_taxes]).all()
        else:
            return sales_taxes
    except Exception as e:
        return CallToolResult(content=[TextContent(type="text", text=str(e))], isError=True)
...
async def get_all_business_partners(
    ...
    try:
        ...
        if jq_filter is not None and jq_filter != "":
            return jq.compile(jq_filter).input_(
...
async def get_all_invoice_templates(jq_filter: Annotated[Optional[str], Field(
    ...
    try:
        ...
        if jq_filter is not None and jq_filter != "":
            return jq.compile(jq_filter).input_value(
...
async def create_position(position: InvoicePosition,
    ...
    try:
        ...
        if jq_filter is not None and jq_filter != "":
            return jq.compile(jq_filter).input_value(created_position.model_dump(mode="json")).all()
        else:
...
async def update_invoice_by_id(id: Annotated[int, Field(description="The id of the invoice")], invoice: Invoice,
    ...
    try:
        ...
        if jq_filter is not None and jq_filter != "":
            return jq.compile(jq_filter).input_value(updated_invoice.model_dump(mode="json")).all()
        else:
...
async def download_file_by_id(id: Annotated[int, Field(description="The id of the file")],
    ...
    try:
        ...
        if jq_filter is not None and jq_filter != "":
            return jq.compile(jq_filter).input_value(file.model_dump(mode="json")).all()
        else:
            return file
    except Exception as e:
        return CallToolResult(content=[TextContent(type="text", text=str(e))], isError=True)
