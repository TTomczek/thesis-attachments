...
async def get_universe_categories_category_id(category_id: Annotated[int, Field(description="An Eve item category ID")],
    ...
    try:
        ...
        if jq_filter is not None and jq_filter != "":
            return CallToolResult(content=[TextContent(type="text", text=jq.compile(jq_filter).input_value(category.model_dump(mode="json")).all())], isError=False)
        else:
            return category
...
async def get_universe_groups_group_id(group_id: Annotated[int, Field(description="An Eve item group ID")],
    ...
    try:
        ...
        if jq_filter is not None and jq_filter != "":
            return CallToolResult(content=[TextContent(type="text", text=jq.compile(jq_filter).input_value(group.model_dump(mode="json")).all())], isError=False)
        else:
            return group
    except Exception as e:
        return CallToolResult(content=[TextContent(type="text", text=str(e))], isError=True)
...
async def post_universe_ids(ids: Annotated[list[str], Field(description="A unique list of names to convert to ids")],
    ...
    try:
        ...
        if jq_filter is not None and jq_filter != "":
            return CallToolResult(content=[TextContent(type="text", text=jq.compile(jq_filter).input_value(names.model_dump(mode="json")).all())], isError=False)
        else:
            return names
    except Exception as e:
        return CallToolResult(content=[TextContent(type="text", text=str(e))], isError=True)
...
async def get_universe_types_type_id(type_id: Annotated[int, Field(description="An Eve item type ID")],
    ...
    try:
        ...
        if jq_filter is not None and jq_filter != "":
            return CallToolResult(content=[TextContent(type="text", text=jq.compile(jq_filter).input_value(types.model_dump(mode="json")).all())], isError=False)
        else:
            return types
    except Exception as e:
        return CallToolResult(content=[TextContent(type="text", text=str(e))], isError=True)
