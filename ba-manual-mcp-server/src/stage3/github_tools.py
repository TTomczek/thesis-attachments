...
from src.github_client.models.repository import Repository
...
Annotated[CallToolResult, List[Issue]]:
    """
    ...
    try:
        issues: List[Issue] = await issuesApi.issues_list_for_repo(owner=owner, repo=repo, milestone=milestone, state=nullsafe_state,
                                                      assignee=assignee, type=issue_type, creator=creator,
                                                      mentioned=mentioned, labels=labels, sort=nullsafe_sort,
                                                      direction=nullsafe_direction, since=since, per_page=per_page,
                                                      page=page)
        return CallToolResult(content=[], structuredContent={"result": [issue.model_dump() for issue in issues]})
...
async def issues_create(owner: Annotated[
    str, Field(description="The account owner of the repository. The name is not case sensitive.")], repo: Annotated[
    str, Field(description="The name of the repository without the `.git` extension. The name is not case sensitive.")],
                        issue: CreateIssueRequest) -> Annotated[CallToolResult, Issue]:
    """
    ...
    try:
        ...
        return CallToolResult(content=[], structuredContent={"result": created_issue.model_dump()})
    except Exception as e:
        return CallToolResult(content=[TextContent(type="text", text=str(e))], isError=True)
...
async def issues_list_labels_for_repo(owner: Annotated[
    ...
    str, Field(description="The name of the repository without the `.git` extension. The name is not case sensitive.")],
                                      ...
                                      page: Annotated[Optional[int], Field(
                                          description="The page number of the results to fetch.")] = 1) -> Annotated[
    CallToolResult, list[Label]]:
    ...
    try:
        ...
        return CallToolResult(content=[], structuredContent={"result": [label.model_dump() for label in labels]})
    except Exception as e:
        return CallToolResult(content=[TextContent(type="text", text=str(e))], isError=True)
...
async def repos_list_branches(owner: Annotated[
    ...
    str, Field(description="The name of the repository without the `.git` extension. The name is not case sensitive.")],
                              ...
                              page: Annotated[
                                  Optional[int], Field(description="The page number of the results to fetch.")] = 1) -> \
Annotated[CallToolResult, List[ShortBranch]]:
    """List branches of a repository."""
    try:
        branches = await reposApi.repos_list_branches(owner=owner, repo=repo, protected=protected, per_page=per_page,
                                                      page=page)
        return CallToolResult(content=[], structuredContent={"result": [branch.model_dump() for branch in branches]})
    except Exception as e:
        return CallToolResult(content=[TextContent(type="text", text=str(e))], isError=True)
...
Annotated[CallToolResult, BranchProtection]:
    """
    ...
    try:
        ...
        return CallToolResult(content=[], structuredContent={"result": protection.model_dump()})
    except Exception as e:
        return CallToolResult(content=[TextContent(type="text", text=str(e))], isError=True)
...
async def activity_list_repos_starred_by_authenticated_user(
        ...
        page: Annotated[Optional[int], Field(description="The page number of the results to fetch.")] = 1) -> Annotated[
    CallToolResult, List[Repository]]:
    """
    ...
    try:
        ...
        return CallToolResult(content=[], structuredContent={"result": [repo.model_dump() for repo in repos]})
    except Exception as e:
        return CallToolResult(content=[TextContent(type="text", text=str(e))], isError=True)
