    ...
    try:
        ...
        if jq_filter is not None and jq_filter != "":
            return CallToolResult(content=[], structuredContent={
                "result": jq.compile(jq_filter).input_(
...
Union[InvManInvoice, Any]:
    ...
    try:
        ...
        if jq_filter is not None and jq_filter != "":
            return CallToolResult(content=[], structuredContent={
                "result": jq.compile(jq_filter).input_value(created_invoice.model_dump(mode="json")).all()})
        else:
            return created_invoice.model_dump(mode="json")
    except Exception as e:
        return CallToolResult(content=[TextContent(type="text", text=str(e))], isError=True)
...
async def get_all_sales_taxes(jq_filter: Annotated[Optional[str], Field(
    description="An optional jq filter to apply to the result, to customize the result format.")] = None) -> Annotated[
    CallToolResult, List[SalesTax]]:
    ...
    try:
        ...
        if jq_filter is not None and jq_filter != "":
            return CallToolResult(content=[], structuredContent={
                "result": jq.compile(jq_filter).input_([tax.model_dump(mode="json") for tax in sales_taxes]).all()})
        else:
            return [sales_tax.model_dump(mode="json") for sales_tax in sales_taxes]
    except Exception as e:
        return CallToolResult(content=[TextContent(type="text", text=str(e))], isError=True)
...
Union[List[BusinessPartner], Any]:
    ...
    try:
        ...
        if jq_filter is not None and jq_filter != "":
            ...
            return [partner.model_dump(mode="json") for partner in business_partners]
    except Exception as e:
        return CallToolResult(content=[TextContent(type="text", text=str(e))], isError=True)
...
async def get_all_invoice_templates(jq_filter: Annotated[Optional[str], Field(
    description="An optional jq filter to apply to the result, to customize the result format.")] = None) -> Annotated[
    CallToolResult, List[InvoiceTemplate]]:
    ...
    try:
        ...
        if jq_filter is not None and jq_filter != "":
            return CallToolResult(content=[], structuredContent={
                "result": jq.compile(jq_filter).input_value(
                    [template.model_dump(mode="json") for template in invoice_templates]).all()})
        else:
            return [template.model_dump(mode="json") for template in invoice_templates]
    except Exception as e:
        return CallToolResult(content=[TextContent(type="text", text=str(e))], isError=True)
...
Union[InvManInvoicePosition, Any]:
    ...
    try:
        ...
        if jq_filter is not None and jq_filter != "":
            return CallToolResult(content=[], structuredContent={
                "result": jq.compile(jq_filter).input_value(created_position.model_dump(mode="json")).all()})
        else:
            return created_position.model_dump(mode="json")
    except Exception as e:
        return CallToolResult(content=[TextContent(type="text", text=str(e))], isError=True)
...
Union[InvManInvoice, Any]:
    ...
    try:
        ...
        if jq_filter is not None and jq_filter != "":
            return CallToolResult(content=[], structuredContent={
                "result": jq.compile(jq_filter).input_value(updated_invoice.model_dump(mode="json")).all()})
        else:
            return updated_invoice.model_dump(mode="json")
    except Exception as e:
        return CallToolResult(content=[TextContent(type="text", text=str(e))], isError=True)
...
@rate_limit()
@sanitize_output()
async def get_invoice_pdf_by_id(id: Annotated[int, Field(description="The id of the invoice")]) -> Annotated[
    CallToolResult, int]:
...
Union[DownloadFile, Any]:
    ...
    try:
        ...
        if jq_filter is not None and jq_filter != "":
            return CallToolResult(content=[], structuredContent={
                "result": jq.compile(jq_filter).input_value(file.model_dump(mode="json")).all()})
        else:
