    ...
    try:
        ...
        if jq_filter is not None and jq_filter != "":
            return CallToolResult(
                content=[],
                structuredContent={"result": jq.compile(jq_filter).input_value(
    ...
    try:
        ...
        if jq_filter is not None and jq_filter != "":
            return CallToolResult(content=[], structuredContent={
                "result": jq.compile(jq_filter).input_value(created_issue.model_dump(mode="json")).all()})
        else:
            return created_issue.model_dump(mode="json")
    except Exception as e:
        return CallToolResult(content=[TextContent(type="text", text=str(e))], isError=True)
...
Union[List[Label], Any]:
    ...
    try:
        ...
        if jq_filter is not None and jq_filter != "":
            return CallToolResult(content=[], structuredContent={"result": jq.compile(jq_filter).input_value(
                [label.model_dump(mode="json") for label in labels]).all()})
        else:
            return [label.model_dump(mode="json") for label in labels]
    except Exception as e:
        return CallToolResult(content=[TextContent(type="text", text=str(e))], isError=True)
...
async def repos_list_branches(owner: Annotated[
    ...
    try:
        ...
        if jq_filter is not None and jq_filter != "":
            ...
            return [branch.model_dump(mode="json") for branch in branches]
    except Exception as e:
        return CallToolResult(content=[TextContent(type="text", text=str(e))], isError=True)
...
async def repos_get_branch_protection(owner: Annotated[
    ...
    try:
        ...
        if jq_filter is not None and jq_filter != "":
            return CallToolResult(content=[], structuredContent={
                "result": jq.compile(jq_filter).input_value(protection.model_dump(mode="json")).all()})
        else:
            return protection.model_dump(mode="json")
    except Exception as e:
        return CallToolResult(content=[TextContent(type="text", text=str(e))], isError=True)
...
async def activity_list_repos_starred_by_authenticated_user(
    ...
    try:
        ...
        if jq_filter is not None and jq_filter != "":
            return CallToolResult(content=[], structuredContent={
                "result": jq.compile(jq_filter).input_value([repo.model_dump(mode="json") for repo in repos]).all()})
        else:
